...
set_packages()
{
    ...
    # SNMP subagent is for local HSMs only, so shouldn't be installed with the Luna SA client
...
prompt_for_components()
{
    ...
    if [ $first_time_in_components -eq 1 ] ; then
        ...
        elif [ "$LUNA_REM_BACKUP_HSM_SELECTED" = "*" ] ; then
            LUNA_SNMP_SELECTED="$SAVED_LUNA_SNMP_SELECTED"
        fi
    fi
    ...
    if [ $first_time_in_components -gt 2 ] ; then
        if [ "$LUNA_SA_SELECTED" = "*" ] || [ "$LUNA_PCI_SELECTED" = "*" ] || [ "$LUNA_G5_SELECTED" = "*" ] ; then
    ...
    if [ "$LUNA_SA_SELECTED" = "*" ] || [ "$LUNA_PCI_SELECTED" = "*" ] || [ "$LUNA_G5_SELECTED" = "*" ] ; then
        ...
        if [ "$LUNA_SA_SELECTED" = "*" ] ; then
            ...
            # FIXME: After 5.3 do the proper fix
            # Temporary solution for 5.3
            # Forcing actions because under some cases ldclient.jar is installed when Luna SA is not selected
            # CCC provisioning client (ldclient.jar file) should not be installed when Luna SA is not selected
            ldpc_client_pkgs=""
            LUNA_DPC_SELECTED=" "
        fi
    fi
...
{   
    service luna-snmp status &> /dev/null
    if [ $? == 0 ]; then
        service luna-snmp stop
    fi
    service htlc_service status &> /dev/null
    if [ $? == 0 ]; then
        service htlc_service stop 
    fi
    service pedclient_service status &> /dev/null
    if [ $? == 0 ]; then
    ...
    if [ "$1" = "remove_client" ] ; then
        ...
        if [ "$components_list" = "" ] ; then
            if [ "$LUNA_PCI_SELECTED" = "*" ] || [ "$LUNA_G5_SELECTED" = "*" ] || [ "$LUNA_SA_SELECTED" = "*" ] ; then
                ...
                if [ "$LUNA_PCI_SELECTED" = "*" ] || [ "$LUNA_G5_SELECTED" = "*" ] ; then
                    LUNA_SNMP_SELECTED="*"
                fi
            elif [ "$LUNA_REM_BACKUP_HSM_SELECTED" = "*" ] ; then
                LUNA_SNMP_SELECTED="*"
...
if [ $opt_uninstall_client -eq 1 ] ; then
    ...
    verify_uninstallable_driver "vkd"
    verify_uninstallable_driver "uhd"
